...
- **CLI core smoke**: :func:`run_paired_training` (the testable core of
  ...
  tiny VAE and writes a checkpoint, with the callbacks wired (PSNR/SSIM,
  train-metrics) and ``val/psnr`` / ``val/ssim`` reported on the RAW optimizer
...
def test_run_paired_training_smoke(tmp_path):
    ...
    wiring PSNR/SSIM + train-metrics and reporting val/psnr+val/ssim on the RAW
    optimizer weights."""
    ...
    # EMA was removed; no DoubleEMACallback must be wired into the trainer.
    assert not any(type(c).__name__ == "DoubleEMACallback" for c in trainer.callbacks)
