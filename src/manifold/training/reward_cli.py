...
def run_reward_training(
    *,
    ...
    The shell (ADR-0032): seed, the pre-fit probe mutation, a stock
    ``ModelCheckpoint`` (monitoring the generated-end probe, or an unmonitored
    fallback under DDP where the metric is rank-local), then delegate to
    :class:`TrainingSpine.run`. Returns ``(trainer, ckpt)`` so callers can find
    the written ``.ckpt``.
