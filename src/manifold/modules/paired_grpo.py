...
def singular_branch_rollout_paired(
    ...
    *,
    ...
    if any(k < 0 for k in eta_steps):
        # A negative index (e.g. from a config typo) slips past the
        # max-only check below (-1 >= num_steps is False) and then
        # indexes nodes[-1] (=1.0, the terminal) with t_next =
        # nodes[0] (=0.0) -> a backwards step that divides by
        # (1-1)=0 (ZeroDivisionError in sde_step_mean). Reject it
        # (codex #107).
        raise ValueError(
