...
(ADR-0034). The shipped evaluation stack also provides deterministic
same-noise before/after comparisons and paired 3D PSNR/SSIM reporting. The
supervised ControlNet path now defaults to an observe-only fixed-subset monitor
that reuses the same full-rollout decode, normalization, and 3D metric contract;
`manifold-eval` remains the separate offline before/after lifecycle. The wiki is
...
- [Before/after GRPO evaluation](evaluation.md) — the active fixed-subset in-training paired-fidelity monitor plus `manifold-eval`, same-noise artifact comparison, paired PSNR/SSIM, slice grids, and report artifacts.
...
| Change or extend the in-training paired-fidelity monitor | [evaluation.md](evaluation.md#active-in-training-paired-fidelity-monitor), [callback-registry.md](callback-registry.md#built-in-specs), [workflows.md](workflows.md#paired-training) | `src/manifold/metrics/{paired_callback.py,paired.py,vae_stage.py,metric_plot_callback.py}`, `src/manifold/training/callbacks/{paired_fidelity.py,registry.py,context.py}`, `src/manifold/training/controlnet_cli.py` | `PairedFidelityCallback`, `PairedFidelitySpec`, `VaeStage`, `LatentDecoder`, `PairedFidelityMetrics`, `CallbackRegistry` | `tests/test_paired_fidelity_callback.py`, `test_callback_registry.py`, `test_fid_helpers.py`, `test_controlnet_cli.py`, `test_paired_fidelity_ddp.py` | `pytest tests/test_paired_fidelity_callback.py tests/test_callback_registry.py -q`; run the 2-rank test when collective behavior changes |
...
| Edit validation / FID / x0-MAE / reward metric | [operations-and-testing.md](operations-and-testing.md#distributed-validation-contract), [callback-registry.md](callback-registry.md) | `src/manifold/metrics/fid/*`, `src/manifold/training/metrics.py`, `src/manifold/training/callbacks/{fid,registry}.py` | `FIDCallback`, `LatentX0MAE`, `PairedFidelityCallback`, `PairedFidelityMetrics`, `FIDSpec`, `CallbackRegistry`, `CallbackContext` | `tests/test_fid.py`, `test_fid_helpers.py`, `test_paired_fidelity.py`, `test_paired_fidelity_callback.py`, `test_metric_plot.py`, `test_ddp_metrics.py`, `test_callback_registry.py`, `test_paired_fidelity_ddp.py` | `pytest tests/test_fid.py tests/test_ddp_metrics.py -q` |
| Add / change a training callback | [callback-registry.md](callback-registry.md) | `src/manifold/training/callbacks/{registry,context,train_loss,fid,paired_fidelity,checkpoint}.py`, `src/manifold/training/core.py` | `CallbackRegistry`, `CallbackSpec`, `PairedFidelitySpec`, `TrainingSpine.run`, `forbidden_callbacks`, `forbidden_monitors` | `tests/test_callback_registry.py`, `test_paired_fidelity_callback.py`, `tests/test_training_cli.py` | `pytest tests/test_callback_registry.py tests/test_paired_fidelity_callback.py -q` |
...
├── docs/adr/                   # 37 ADRs; ADR-0037 is the latest (paired-fidelity monitor decision)
...
- **Paired fidelity/eval:** preserve `min_max_to_unit` → `PairedFidelityMetrics(data_range=1.0)` across the ControlNet rollout, in-training callback, offline driver, and report. The in-training path is observe-only and uses the existing fixed-subset checkpoint, generation seed, full Heun rollout, and decode stages; changing it crosses the module, metrics, callback spec, CLI registration, and DDP tests together.
- **Metrics:** distinguish per-rank accumulation from global reduction. Manual `all_reduce` must not also use `sync_dist=True` for the same value. The paired-fidelity callback is default-on for supervised ControlNet: it redundantly evaluates the same fixed subset on every rank, logs `val/psnr` and `val/ssim` through module-attached `MeanMetric`s, and leaves `val/x0_mae` as the default checkpoint monitor.
...
`CONTEXT.md` still labels the ADR-0029–0033 callback/device refactor as pending in its "Training callbacks & device ownership" section. Treat that paragraph as stale: the registry/spine and `DevicePolicy` implementations in this wiki are the active current contract.
...
- Wire the `paired_fidelity` recipe block into the shipped `manifold-train-controlnet` callback configuration if those monitor knobs are intended to be user-configurable. `PairedFidelitySpec` accepts `subset_size`, `every_n_epochs`, and `seed` through `callback_cfg`, and the CLI forwards `controlnet.num_inference_steps` as the recipe-primary rollout count, but `controlnet_cli.main()` currently passes only the `checkpoint` block as `callback_cfg`; there is no direct CLI reading of the other `paired_fidelity` keys.